...
//    File url = new File ("C:\\lastURL.txt");
    File url = new File ("C:\\Users\\lukas\\lastURL.txt");
    public Splash() {
        initComponents();
    }
    ...
    public static ArrayList<text> bodyFind(String htmlText) {
        ...
        boolean isUndr = false;
        
        while( true ){
            String tag = "";
            indexOfStart = htmlTextRemain.indexOf( '<' );
            if( indexOfStart == -1 ) break;
            String lastText = htmlTextRemain.substring( 0, indexOfStart );
            paragraphs.add( new text( lastText, isAddr, isBold, isUndr ));
            
            
            // All count as valid tags
            // <b class="foobar">
            // <b>
            // <br>
            indexOfEnd = htmlTextRemain.indexOf( '>' );
            tag = htmlTextRemain.substring( indexOfStart+1, indexOfEnd-1 );
            htmlTextRemain = htmlTextRemain.substring( indexOfEnd+1 );

            indexOfEnd = tag.indexOf( ' ' );
            if( indexOfEnd != -1 ) tag = tag.substring( 0, indexOfEnd-1 );
            ...
            switch (tag) {
                ...
                case "h1": // fall through (Ignore other header cases)
